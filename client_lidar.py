"""
LiDAR客户端 - 专门处理激光雷达数据
基于 torch.distributed 的分布式训练节点（支持单机模式）
"""
import torch
...
#         print(f"[LiDAR Client] Rank {rank}/{world_size} 分布式初始化成功")
#         return True
#     except Exception as e:
#         print(f"[LiDAR Client] 分布式初始化失败: {e}")
#         print("  使用单机模式继续训练...")
#         return False
...
def cleanup():
    ...
    try:
        if dist.is_initialized():
            ...
            print("[LiDAR Client] 分布式环境已清理")
    except:
        pass  # 如果分布式未初始化，忽略错误

def load_client_data(dataset_dir, client_type='lidar', data_ratio=0.5):
    """
    加载客户端数据 - LiDAR客户端只加载LiDAR数据
    data_ratio: 客户端拥有的数据比例 (0-1)
    """
    print(f"[LiDAR Client] 加载 {client_type.upper()} 数据 (比例: {data_ratio})...")
...
def train_client(rank, world_size, args):
    """LiDAR客户端训练函数"""
    print(f"\n{'='*60}")
    print(f"LiDAR Client - Rank {rank}/{world_size}")
    print(f"{'='*60}")

    # 设置分布式
    dist_initialized = setup(rank, world_size, port=args.port)

    # 设备设置
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"[LiDAR Client] 使用设备: {device}")
    ...
    for round in range(args.fed_rounds):
        if rank == 0:
            print(f"\n[LiDAR Client] 联邦轮次 {round+1}/{args.fed_rounds}")
        ...
        for epoch in range(args.local_epochs):
            ...
            if rank == 0 and (epoch+1) % 2 == 0:
                print(f"[LiDAR Client]  本地Epoch {epoch+1}/{args.local_epochs}: "
        ...
        if rank == 0:
            print(f"[LiDAR Client] 联邦轮次 {round+1} 完成: "
        ...
        try:
            if args.sync_interval > 0 and (round+1) % args.sync_interval == 0:
                ...
                if dist.is_initialized():
                    ...
                    print(f"[LiDAR Client] 单机模式，跳过参数同步")
        except:
            print(f"[LiDAR Client] 参数同步失败，继续训练")

    # 保存客户端模型
    if rank == 0:
        model_path = f"./output/client_lidar_model_round{args.fed_rounds}.pth"
        torch.save(model.state_dict(), model_path)
        print(f"\n[LiDAR Client] ✅ 客户端模型已保存: {model_path}")

    # 清理
    cleanup()
    print(f"[LiDAR Client - Rank {rank}] 训练完成")

def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='LiDAR联邦学习客户端')
